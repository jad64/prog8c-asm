...
;---------------------------------------------------------------
D6510           = $00   ;   7-0     MOS 6510 DATA DIRECTION
                        ;                   REGISTER (XX101111)
                        ;                   BIT= 1: OUTPUT, BIT=0:
                        ;                   INPUT, X=DON'T CARE
R6510           = $01   ;           MOS 6510 MICRO-PROCESSOR
                        ;                   ON-CHIP I/O PORT
...
MSIGX           = $D010 ;           SPRITES 0-7 X POS (MSB OF X COORD.)
...
RASTER          = $D012 ;           READ RASTER / WRITE RASTER VALUE FOR COMPARE IRQ
LPENX           = $D013 ;           LIGHT-PEN LATCH X POS
LPENY           = $D014 ;           LIGHT-PEN LATCH Y POS
SPENA           = $D015 ;           SPRITE DISPLAY ENABLE: 1 = ENABLE
...
YXPAND          = $D017 ;           SPRITES 0-7 EXPAND 2X VERTICAL (Y)
...
VICIRQ          = $D019 ;           VIC INTERRUPT FLAG REGISTER (BIT = 1: IRQ OCCURRED)
...
IRQMSK          = $D01A ;           IRQ MASK REGISTER: 1 = INTERRUPT ENABLED
SPBGPR          = $D01B ;           SPRITE TO BACKGROUND DISPLAY PRIORITY: 1 = SPRITE
SPMC            = $D01C ;           SPRITES 0-7 MULTI-COLOR MODE SELECT: 1 = M.C.M.
XXPAND          = $D01D ;           SPRITES 0-7 EXPAND 2X HORIZONTAL (X)
...
ATDCY1          = $D405 ;           ENVELOPE GENERATOR 1: ATTACK / DECAY CYCLE CONTROL
                        ;   7-4     SELECT ATTACK CYCLE DURATION: 0-15
                        ;   3-0     SELECT DECAY CYCLE DURATION: 0-15
SUREL1          = $D406 ;           ENVELOPE GENERATOR 1: SUSTAIN / RELEASE CYCLE CONTROL
...
VCREG2          = $D40B ;           VOICE 2: CONTROL REGISTER
                        ...
                        ;   0       GATE BIT: 1 = START ATT/DEC/SUS, 0 = START RELEASE
ATDCY2          = $D40C ;           ENVELOPE GENERATOR 2: ATTACK / DECAY CYCLE CONTROL
                        ;   7-4     SELECT ATTACK CYCLE DURATION: 0-15
                        ;   3-0     SELECT DECAY CYCLE DURATION: 0-15
SUREL2          = $D40D ;           ENVELOPE GENERATOR 2: SUSTAIN / RELEASE CYCLE CONTROL
                        ;   7-4     SELECT SUSTAIN CYCLE DURATION: 0-15
                        ;   3-0     SELECT RELEASE CYCLE DURATION: 0-15
...
VCREG3          = $D412 ;           VOICE 3: CONTROL REGISTER
                        ...
                        ;   0       GATE BIT: 1 = START ATT/DEC/SUS, 0 = START RELEASE
ATDCY3          = $D413 ;   ENVELOPE GENERATOR 3: ATTAC/DECAY CYCLE CONTROL
                        ;   7-4     SELECT ATTACK CYCLE DURATION: 0-15
                        ;   3-0     SELECT DECAY CYCLE DURATION: 0-15
SUREL3          = $D414 ;           ENVELOPE GENERATOR 3: SUSTAIN / RELEASE CYCLE CONTROL
                        ;   7-4     SELECT SUSTAIN CYCLE DURATION: 0-15
                        ;   3-0     SELECT RELEASE CYCLE DURATION: 0-15
CUTLO           = $D415 ;           FILTER CUTOFF FREQUENCY: LOW-NYBBLE (BITS 2-0)
CUTHI           = $D416 ;           FILTER CUTOFF FREQUENCY: HIGH-BYTE
RESON           = $D417 ;           FILTER RESONANCE CONTROL / VOICE INPUT CONTROL
...
POTX            = $D419 ;           ANALOG/DIGITAL CONVERTER: GAME PADDLE 1 (0-255)
POTY            = $D41A ;           ANALOG/DIGITAL CONVERTER GAME PADDLE 2 (0-255)
RANDOM          = $D41B ;           OSCILLATOR 3 RANDOM NUMBER GENERATOR
ENV3            = $D41C ;           ENVELOPE GENERATOR 3 OUTPUT
COLOR           = $D800 ;-DBFF       55296-56319     COLOR RAM (NYBBLES)
CIAPRA          = $DC00 ;           DATA PORT A (KEYBOARD, JOYSTICK, PADDLES, LIGHT-PEN)
                        ;
...
CIAPRB          = $DC01 ;           DATA PORT B (KEYBOARD, JOYSTICK, PADDLES): GAME PORT 1
...
CIDDRA          = $DC02 ;           DATA DIRECTION REGISTER - PORT A (56320)
CIDDRB          = $DC03 ;           DATA DIRECTION REGISTER - PORT B (56321)
TIMALO          = $DC04 ;           TIMER A: LOW-BYTE
TIMAHI          = $DC05 ;           TIMER A: HIGH-BYTE
TIMBLO          = $DC06 ;           TIMER B: LOW-BYTE
TIMBHI          = $DC07 ;           TIMER B: HIGH-BYTE
TODTEN          = $DC08 ;           TIME-OF-DAY CLOCK: 1/10 SECONDS
TODSEC          = $DC09 ;           TIME-OF-DAY CLOCK: SECONDS
TODMIN          = $DC0A ;           TIME-OF-DAY CLOCK: MINUTES
TODHRS          = $DC0B ;           TIME-OF-DAY CLOCK: HOURS + AM/PM FLAG (BIT 7)
CIASDR          = $DC0C ;           SYNCHRONOUS SERIAL I/O DATA BUFFER
CIAICR          = $DC0D ;           CIA INTERRUPT CONTROL REGISTER (READ IRQS/WRITE MASK)
...
CI2PRA          = $DD00 ;           DATA PORT A (SERIAL BUS, RS-232, VIC MEMORY CONTROL)
...
CI2PRB          = $DD01 ;   DATA PORT B (USER PORT, RS-232)
...
C2DDRA          = $DD02 ;           DATA DIRECTION REGISTER - PORT A
C2DDRB          = $DD03 ;           DATA DIRECTION REGISTER - PORT B
TI2ALO          = $DD04 ;           TIMER A: LOW-BYTE
TI2AHI          = $DD05 ;           TIMER A: HIGH-BYTE
TI2BLO          = $DD06 ;           TIMER B: LOW-BYTE
TI2BHI          = $DD07 ;           TIMER B: HIGH-BYTE
TO2TEN          = $DD08 ;           TIME-OF-DAY CLOCK: 1/10 SECONDS
TO2SEC          = $DD09 ;           TIME-OF-DAY CLOCK: SECONDS
TO2MIN          = $DD0A ;           TIME-OF-DAY CLOCK: MINUTES
TO2HRS          = $DD0B ;           TIME-OF-DAY CLOCK: HOURS + AM/PM FLAG (BIT 7)
CI2SDR          = $DD0C ;           SYNCHRONOUS SERIAL I/O DATA BUFFER
CI2ICR          = $DD0D ;           CIA INTERRUPT CONTROL REGISTER (READ NMLS/WRITE MASK)
                        ;   7       NMI FLAG (1 = NMI OCCURRED) / SET-CLEAR FLAG
                        ;   4       FLAG1 NMI (USER/RS-232 RECEIVED DATA INPUT)
                        ;   3       SERIAL PORT INTERRUPT
                        ;   1       TIMER B INTERRUPT
                        ;   0       TIMER A INTERRUPT
CI2DRA          = $DD0E ;           CIA CONTROL REGISTER A
                        ;   7       TIME-OF-DAY CLOCK FREQUENCY: 1 = 50 HZ, 0 = 60 HZ
                        ;   6       SERIAL PORT I/O MODE OUTPUT, 0 = INPUT
                        ;   5       TIMER A COUNTS: 1 = CNT SIGNALS, 0 = SYSTEM 02 CLOCK
                        ;   4       FORCE LOAD TIMER A: 1 = YES
